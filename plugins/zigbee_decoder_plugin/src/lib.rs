...
fn decode_chip(seq: u32, threshold: u32) -> Option<u8> {
...
#[derive(Block)]
#[message_outputs(out)]
pub struct ZigbeeDecoder<I = DefaultCpuReader<f32>>
where
...
impl<I> Kernel for ZigbeeDecoder<I>
where
    I: CpuBufferReader<Item = f32>,
{
    async fn work(
        ...
        for v in inbuf.into_iter() {
            ...
            match &mut self.state {
                ...
                State::SearchHeader { byte } => {
                    if self.chip_count == 0 {
                        if let Some(i) =
                            decode_chip(self.correlator.shift_reg, self.correlator.threshold)
                        {
                            if let Some(o) = byte {
                                let len = (i << 4) | *o;
                                if len < 128 {
...
plugin_api::export_plugin! {
    name: "ZigbeeDecoder",
    description: "Zigbee chip correlator and frame decoder",
    config: u32,
    create: |cfg, _id| {
        ZigbeeDecoder::<DefaultCpuReader<f32>>::new(cfg)
    }
}
