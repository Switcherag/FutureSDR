    ...
    I: CpuBufferReader<Item = f32>,
{
    async fn work(
        ...
        for v in inbuf.into_iter() {
            // Accumulate symbol for constellation view (as complex with real part only)
            self.syms.push(Complex32::new(v, 0.0));
        ...
        // Send accumulated symbols in batch
