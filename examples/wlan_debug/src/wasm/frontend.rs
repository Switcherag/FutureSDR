...
pub fn Wlan(fg_handle: FlowgraphHandle) -> impl IntoView {
    ...
    let tx_gain_label = NodeRef::<Span>::new();
    let rx_gain_label = NodeRef::<Span>::new();
    ...
    view! {
        <div class="border-2 border-slate-500 rounded-md flex flex-row flex-wrap m-4 p-4">
            ...
            <div class="basis-1/3 text-white">
                <span class="block mb-2">"Sample Rate"</span>
                <div class="flex flex-row gap-2">
                    {[0.5e6, 1e6, 2e6, 4e6, 5e6, 10e6, 20e6].into_iter().map(|rate| {
                        let fg_handle = fg_handle.clone();
                            ...
                            <button class="p-1 bg-slate-700 hover:bg-slate-600 rounded"
                                on:click=move |_| {
                                    ...
                                    spawn_local(async move {
                                        let _ = fg_handle.call(6, "sample_rate", p.clone()).await;
                                        let _ = fg_handle.call(8, "sample_rate", p).await;
                                    });
                                }
                            >{label}</button>
                        }
            ...
            // TX Gain Slider (SeifySink = block 6)
            <div class="basis-1/2 mt-4">
                <span class="text-white block mb-2">"TX Gain"</span>
                <input type="range" min="0" max="89" value="80" class="align-middle w-64"
                    on:change= {
                        let fg_handle = fg_handle.clone();
                        move |v| {
                            let target = v.target().unwrap();
                            let input : HtmlInputElement = target.dyn_into().unwrap();
                            tx_gain_label.get().unwrap().set_inner_text(&format!("TX: {} dB", input.value()));
                            let gain : f64 = input.value().parse().unwrap();
                            let p = Pmt::F64(gain);
                            let mut fg_handle = fg_handle.clone();
                            spawn_local(async move {
                                let _ = fg_handle.call(6, "gain", p).await;
                            });
                }} />
                <span class="text-white p-2 m-2" node_ref=tx_gain_label>"TX: 80 dB"</span>
            </div>

            // RX Gain Slider (SeifySource = block 8)
            <div class="basis-1/2 mt-4">
                <span class="text-white block mb-2">"RX Gain"</span>
                <input type="range" min="0" max="73" value="50" class="align-middle w-64"
                    on:change= {
                        let fg_handle = fg_handle.clone();
                        move |v| {
                            let target = v.target().unwrap();
                            let input : HtmlInputElement = target.dyn_into().unwrap();
                            rx_gain_label.get().unwrap().set_inner_text(&format!("RX: {} dB", input.value()));
                            let gain : f64 = input.value().parse().unwrap();
                            let p = Pmt::F64(gain);
                            let mut fg_handle = fg_handle.clone();
                            spawn_local(async move {
                                let _ = fg_handle.call(8, "gain", p).await;
                            });
                }} />
                <span class="text-white p-2 m-2" node_ref=rx_gain_label>"RX: 50 dB"</span>
            </div>
        </div>

        <div class="border-2 border-slate-500 rounded-md m-4" style="height: 800px; max-height: 90vh">
            <ConstellationSinkDensity width=width bins=512 decay=0.995 intensity=0.15 />
        </div>
