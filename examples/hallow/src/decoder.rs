    ...
    I: CpuBufferReader<Item = u8>,
{
    pub fn new() -> Self {
        Self {
            input: I::default(),
            frame_complete: true,
            frame_param: FrameParam::new(Mcs::Mcs0, 0),
    ...
    fn deinterleave(&mut self) {
        ...
        // 1 MHz interleaver uses N_col=8, N_row=3 for BPSK
        // General formula from spec (same structure as 802.11a but with N_DATA_SC)
        for j in 0..n_cbps {
            first[j] = s * (j / s) + ((j + (N_DATA_SC * j / n_cbps)) % s);
        }
        for i in 0..n_cbps {
            second[i] = N_DATA_SC * i - (n_cbps - 1) * (N_DATA_SC * i / n_cbps);
        }
    ...
    fn descramble(&mut self) {
        ...
        // 802.11ah uses 8-bit SERVICE field (not 16 like 802.11a/g)
        for i in 7..self.frame_param.psdu_size() * 8 + N_SERVICE_BITS {
