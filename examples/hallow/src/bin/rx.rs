...
fn main() -> Result<()> {
    ...
    let (prev, output): (BlockId, &str) = if let Some(center_freq) = args.capture_center_freq {
        ...
        if args.decimate > 1 {
            // Combined frequency shift + decimating FIR filter
            ...
            // Frequency shift only (no decimation)
        ...
        // Decimation only (no frequency shift)
    ...
    // Autocorrelation: x[n] * conj(x[n-GI]) → moving average (N_DATA_SC=24)
    ...
    // Normalization: |autocorrelation| / power
    ...
    // Maps raw signal to Complex32(time_within_symbol, amplitude) for density display
    ...
    let eye_mapper = fg.add_block(Apply::<_, _, _>::new(move |c: &Complex32| -> Complex32 {
        ...
        let x = t * 2.0 - 1.0; // map [0,1] to [-1,1]
        eye_count += 1;
    ...
    // === Output: UDP sinks + message pipe for console ===
    ...
    rt.block_on(async move {
        while let Some(x) = rx_frame.next().await {
            match x {
                Pmt::Blob(data) => {
                    println!("received HaLow frame ({:?} bytes)", data.len());
                }
                _ => break,
            }
        }
