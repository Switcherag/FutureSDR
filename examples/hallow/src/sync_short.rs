...
const MIN_GAP: usize = 240;
const MAX_SAMPLES: usize = 540 * SYMBOL_SAMPLES; // 540 symbols * 40 samples/symbol
    ...
    async fn work(
        ...
        while i < n_input && o < out.len() {
            match self.state {
                ...
                State::Found => {
                    if in_cor[i] > THRESHOLD {
                        // GI for 1 MHz HaLow = 8 samples
                        let f_offset = -in_abs[i].arg() / GI_SAMPLES as f32;
                        self.state = State::Copy(0, f_offset, false);
                        tags.add_tag(o, Tag::NamedF32("halow_start".to_string(), f_offset));
                    } else {
