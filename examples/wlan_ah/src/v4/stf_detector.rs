...
use crate::v4::helpers::{MAX_FRAME_LEN, TS, TU, frame_len_for_psdu};
    ...
    I: CpuBufferReader<Item = Complex32>,
{
    ...
    /// One past the newest sample the detector has *processed*.
    ///
    /// In v2 the ring held exactly the processed samples, so this was
    /// `sample_base + sample_ring.len()`. v4 pushes the whole input buffer into
    /// the ring up front, so the ring runs ahead of the correlator; reporting
    /// `sample_total` keeps every availability/lookahead check identical to v2.
    ...
    I: CpuBufferReader<Item = Complex32>,
{
    async fn work(
        ...
        // Move the input straight into the sample ring. v2 allocated a `Vec`
        // copy of the entire input buffer on every call, at 4 MSps, whether or
        // not a frame was present; the samples have to land in the ring anyway.
        let input_len = {
            let input = self.input.slice();
            self.sample_ring.extend(input.iter().copied());
            input.len()
        };
        self.input.consume(input_len);

        // `sample_total` still advances one sample per iteration, so the
        // correlator and the detection gating see exactly the v2 cadence.
        for _ in 0..input_len {
            let x = self.sample_ring[self.sample_total - self.sample_base];
            self.sample_total += 1;
