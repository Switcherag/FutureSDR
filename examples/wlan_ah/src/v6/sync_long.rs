//! Fork of `examples/wlan/src/sync_long.rs`, retimed for S1G.
//!
//! Same design: correlate against the time-domain long training field over a
//! search window, take the two strongest peaks, derive the fine CFO from the
//! phase between them, emit the two LTF symbols for channel estimation, then
//! strip cyclic prefixes symbol by symbol.
//!
//! 11a correlates 64 samples over a 320-sample window and the LTS repeats
//! every 64. S1G's LTF repeats every 128 (the two FFT windows in the LTF block
//! sit 128 apart), so every length doubles. The reference sequence is derived
//! from `LTF_FREQ` by inverse DFT rather than transcribed, so it cannot drift
//! from the one `FrameEqualizer` equalises against.

use futuresdr::prelude::*;

use crate::v6::helpers::ltf_time;
use crate::{CP_LEN, FFT_SIZE, SYMBOL_LEN};

/// Doubled from 11a's 320 — the LTF period is 128 here, not 64.
const SEARCH_WINDOW: usize = 640;
...
impl Correlator {
    ...
    fn sync(&mut self, input: &[Complex32]) -> (usize, f32) {
        debug_assert_eq!(input.len(), SEARCH_WINDOW + FFT_SIZE - 1);

        for i in 0..SEARCH_WINDOW {
            let mut sum = Complex32::new(0.0, 0.0);
            for k in 0..FFT_SIZE {
                sum += input[i + k] * self.reference[k].conj();
            }
            self.cor[i] = sum;
        }
    ...
    O: CpuBufferWriter<Item = Complex32>,
{
    async fn work(
        ...
        let mut m = input.len().min(out.len());
        ...
        match self.state {
            State::Broken => {
                if m > 0 {
                    // Upstream sends a `wifi_start` tag before any payload, so
                    // reaching here means the tag was lost; resync rather than
                    // emit garbage.
                    self.input.consume(m);
                    return Ok(());
                }
            ...
            State::Sync(freq_offset_short) => {
                // Two LTF symbols follow the correlation peak, back to back.
                let ltf_span = 2 * FFT_SIZE;
                if m >= SEARCH_WINDOW + ltf_span {
                    ...
                    self.input.consume(offset + ltf_span);
                    self.output.produce(ltf_span);
                    io.call_again = true;
                    ...
                    for k in 0..FFT_SIZE {
                        out[i * FFT_SIZE + k] = input[i * SYMBOL_LEN + CP_LEN + k]
                            * Complex32::from_polar(
                                1.0,
                                ((n_copied + i) * SYMBOL_LEN + ltf_span + CP_LEN + k) as f32
                                    * freq_offset,
                            );
                    }
                }
