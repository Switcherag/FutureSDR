...
fn main() -> Result<()> {
    ...
    // ── SyncLongV2 diagnostics ─────────────────────────────────────────
    // sync_info — ws://127.0.0.1:9020
    //   [pre_len, stf_start, sto_int, sto_frac, cfo, f_short]
    let sync_info_ws = WebsocketPmtSink::new(9020);
    // Time-domain LTF samples post-CFO/STO — ws://127.0.0.1:9021
    let ltf_td_ws = WebsocketPmtSink::new(9021);
    connect!(fg, sync_long.sync_info | r#in.sync_info_ws;
