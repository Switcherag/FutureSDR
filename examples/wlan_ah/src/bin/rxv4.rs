//! rxv4 — 802.11ah receiver built from the per-step `v4` pipeline.
//!
//! Identical to `rxv2` apart from the pipeline module: `v4` is `v2` with the
//! per-frame `FrameCtx` clones and the per-`work()` input copy removed.
//!
//!   seify source
...
fn main() -> Result<()> {
    ...
    // between v4 blocks are messages on the `frame` port.
    connect!(fg,
        src.outputs[0] > stf;
        stf.frame | frame.cfo;
        cfo.frame | frame.sto;
