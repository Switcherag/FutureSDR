...
stage_size() {
    ...
    # Fused receiver 360 KB, examples/wlan's front end block by block
    # (about 220 KB), the receiver as one block (the four kernels again, on
    # in-memory buffers: about 90 KB); planning an FFT in the plugin would
    # add 0.8 MB.
    check_size "wlan plugin (802.11a and ah)" "$wlan" 750000
