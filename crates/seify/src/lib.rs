...
pub enum Driver {
    ...
    AaroniaHttp,
    BladeRf,
    /// BladeRF 2.0 (Micro). Self-contained seify backend at
    /// `impls/bladerf2.rs` — distinct from `BladeRf` (which is the
    /// bladerf1 wrapper around `libbladerf-rs`).
    BladeRf2,
    Dummy,
    HackRf,
    RtlSdr,
...
impl FromStr for Driver {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s == "bladerf2" || s == "bladerf-2" || s == "bladerf_2" {
            return Ok(Driver::BladeRf2);
        }
...
pub fn enumerate_with_args<A: TryInto<Args>>(a: A) -> Result<Vec<Args>, Error> {
    ...
    #[cfg(all(feature = "bladerf2", not(target_arch = "wasm32")))]
    {
        // Driver::BladeRf2 is opt-in only via an explicit `driver=bladerf2`
        // arg — we don't enumerate it when `driver` is None, so existing
        // wildcard probes (which hit bladerf1) keep their behavior.
        if matches!(driver, Some(Driver::BladeRf2)) {
            devs.append(&mut impls::BladeRf2::probe(&args)?)
        }
    }
    #[cfg(not(all(feature = "bladerf2", not(target_arch = "wasm32"))))]
    {
        if matches!(driver, Some(Driver::BladeRf2)) {
            return Err(Error::FeatureNotEnabled);
        }
